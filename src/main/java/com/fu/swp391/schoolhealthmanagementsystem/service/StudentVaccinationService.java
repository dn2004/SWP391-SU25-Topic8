...
public class StudentVaccinationService {
    ...
    public StudentVaccinationResponseDto addVaccination(Long studentId, StudentVaccinationRequestDto dto) {
        log.info("[TIÊM CHỦNG] Bắt đầu thêm thông tin tiêm chủng cho học sinh ID: {}", studentId);
        ...
        if (!isStudentActive(student)) {
            log.warn("[TIÊM CHỦNG] Học sinh ID: {} không hoạt động, không thể thêm thông tin tiêm chủng.", studentId);
            throw new InvalidOperationException("Học sinh không hoạt động, không thể thêm thông tin tiêm chủng.");
        }
        ...
        vaccinationEntity.setCreatedByUser(currentUser);
        vaccinationEntity.setApprovedByUser(approvedByUser);

        log.info("[TIÊM CHỦNG] Gán trạng thái mặc định {} cho bản ghi tiêm chủng mới của học sinh ID: {}", vaccinationStatus, studentId);
        ...
        log.info("[TIÊM CHỦNG] Đã lưu bản ghi tiêm chủng với ID: {} và trạng thái: {}", savedEntity.getStudentVaccinationId(), savedEntity.getStatus());

        if (currentUser.getRole() == UserRole.Parent) {
            String content = String.format("Có hồ sơ tiêm chủng mới cho học sinh '%s' cần được duyệt.", student.getFullName());
            String link = "/admin/vaccinations/pending";
            notificationService.createAndSendNotificationToRole(UserRole.MedicalStaff, content, link, currentUser.getEmail());
            log.info("[TIÊM CHỦNG] Đã gửi thông báo cho MedicalStaff về hồ sơ tiêm chủng mới của học sinh ID: {}", studentId);
        }

        return vaccinationMapper.toDto(savedEntity);
    }

    boolean isStudentActive(Student student) {
        return student != null && student.getStatus() == StudentStatus.ACTIVE;
    }
    ...
    public Page<StudentVaccinationResponseDto> getAllVaccinations(String studentName, String vaccineName, LocalDate fromDate, LocalDate toDate, StudentVaccinationStatus status, Pageable pageable) {
        log.info("[TIÊM CHỦNG] Lấy tất cả danh sách tiêm chủng phân trang với bộ lọc: tên học sinh='{}', vaccine='{}', từ ngày={}, đến ngày={}, trạng thái={}", studentName, vaccineName, fromDate, toDate, status);
        ...
        log.info("[TIÊM CHỦNG] Tìm thấy {} bản ghi tiêm chủng trên trang {}.", vaccinationsEntityPage.getNumberOfElements(), pageable.getPageNumber());
    ...
    public Page<StudentVaccinationResponseDto> getAllVaccinationsByStudentIdPage(Long studentId, String vaccineName, LocalDate fromDate, LocalDate toDate, StudentVaccinationStatus status, Pageable pageable) {
        log.info("[TIÊM CHỦNG] Lấy danh sách tiêm chủng phân trang cho học sinh ID: {}. Trang: {}", studentId, pageable.getPageNumber());
        User currentUser = authorizationService.getCurrentUserAndValidate();
        Student student = studentRepository.findById(studentId)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy học sinh với ID: " + studentId));
        ...
        if (currentUser.getRole() == UserRole.Parent) {
            ...
            status = StudentVaccinationStatus.APPROVE;
        }
        ...
        Page<StudentVaccination> vaccinationsEntityPage = vaccinationRepository.findAll(spec, pageable);
        log.info("[TIÊM CHỦNG] Tìm thấy {} bản ghi tiêm chủng cho học sinh ID {} trên trang {}.", vaccinationsEntityPage.getNumberOfElements(), studentId, pageable.getPageNumber());
        return vaccinationsEntityPage.map(vaccinationMapper::toDto);
    }

    @Transactional(readOnly = true)
    public Page<StudentVaccinationResponseDto> getPendingVaccinations(Pageable pageable) {
        log.info("[TIÊM CHỦNG] Lấy danh sách tiêm chủng đang chờ duyệt (PENDING) cho trang: {}. Sắp xếp: {}", pageable.getPageNumber(), pageable.getSort());
        ...
        log.info("[TIÊM CHỦNG] Tìm thấy {} bản ghi tiêm chủng đang chờ duyệt trên trang {}.",
    ...
    public Page<StudentVaccinationResponseDto> getAllVaccinationsByStatus(StudentVaccinationStatus status, Pageable pageable) {
        log.info("[TIÊM CHỦNG] Lấy danh sách tiêm chủng theo trạng thái: {} cho trang: {}. Kích thước trang: {}", status, pageable.getPageNumber(), pageable.getPageSize());
        ...
        log.info("[TIÊM CHỦNG] Tìm thấy {} bản ghi tiêm chủng với trạng thái {} trên trang {}.",
    ...
    public Page<StudentVaccinationResponseDto> getVaccinationsByStudentIdAndStatus(
        ...
        log.info("[TIÊM CHỦNG] Lấy danh sách tiêm chủng cho học sinh ID: {} theo trạng thái: {}. Trang: {}, Kích thước: {}",
        ...
        log.info("[TIÊM CHỦNG] Tìm thấy {} bản ghi tiêm chủng cho học sinh ID {} với trạng thái {} trên trang {}.",
    ...
    public StudentVaccinationResponseDto getVaccinationResponseByIdForCurrentUser(Long vaccinationId) {
        log.info("[TIÊM CHỦNG] Đang lấy thông tin bản ghi tiêm chủng ID: {} cho người dùng hiện tại.", vaccinationId);
        User currentUser = authorizationService.getCurrentUserAndValidate();
    ...
    @Transactional
    public StudentVaccinationResponseDto updateVaccinationForCurrentUser(Long vaccinationId, StudentVaccinationUpdateRequestDto dto) {
        log.info("[TIÊM CHỦNG] Bắt đầu cập nhật bản ghi tiêm chủng ID: {} bởi người dùng hiện tại.", vaccinationId);
        User currentUser = authorizationService.getCurrentUserAndValidate();
        ...
        if (studentOfRecord == null) {
            log.warn("[TIÊM CHỦNG] Bản ghi tiêm chủng ID: {} không liên kết với học sinh nào.", vaccinationId);
            throw new InvalidOperationException("Bản ghi tiêm chủng không liên kết với học sinh.");
        }
        Long studentId = studentOfRecord.getId();
        ...
        if (currentUser.getRole() == UserRole.Parent) {
            ...
            if (existingVaccination.getStatus() != StudentVaccinationStatus.PENDING) {
                log.warn("[TIÊM CHỦNG] Phụ huynh {} cố cập nhật hồ sơ tiêm chủng ID {} không ở trạng thái PENDING.", currentUser.getEmail(), vaccinationId);
                throw new AccessDeniedException("Bạn chỉ có thể cập nhật thông tin tiêm chủng khi đang ở trạng thái 'Chờ xử lý'.");
            }
        } else {
            if (existingVaccination.getStatus() == StudentVaccinationStatus.PENDING) {
                log.warn("[TIÊM CHỦNG] Nhân viên {} cố cập nhật hồ sơ tiêm chủng ID {} ở trạng thái PENDING.", currentUser.getEmail(), vaccinationId);
        ...
        vaccinationMapper.updateEntityFromDto(dto, existingVaccination);
        ...
        if (newProofFile != null && !newProofFile.isEmpty()) {
            if (existingVaccination.getProofPublicId() != null && !existingVaccination.getProofPublicId().isEmpty()) {
                try {
                    ...
                    log.error("[TIÊM CHỦNG] Lỗi xóa file cũ trên Cloudinary: {}", e.getMessage());
                }
            }
        ...
        if (currentUser.getRole() == UserRole.Parent) {
            ...
            log.info("[TIÊM CHỦNG] Bản ghi ID {} được cập nhật bởi phụ huynh, giữ/chuyển về trạng thái PENDING.", vaccinationId);
        } else {
            ...
            log.info("[TIÊM CHỦNG] Bản ghi ID {} được cập nhật bởi nhân viên, tự động chuyển sang trạng thái APPROVE.", vaccinationId);
        }

        StudentVaccination updatedEntity = vaccinationRepository.save(existingVaccination);
        log.info("[TIÊM CHỦNG] Đã cập nhật bản ghi tiêm chủng ID: {}, trạng thái mới: {}", updatedEntity.getStudentVaccinationId(), updatedEntity.getStatus());
       ...
       if (currentUser.getRole() == UserRole.Parent) {
            ...
            String link = "/admin/vaccinations/pending";
            notificationService.createAndSendNotificationToRole(UserRole.MedicalStaff, content, link, currentUser.getEmail());
            log.info("[TIÊM CHỦNG] Đã gửi thông báo cho MedicalStaff về việc cập nhật hồ sơ tiêm chủng của học sinh ID: {}", studentId);
        } else if (currentUser.getRole() == UserRole.MedicalStaff) {
            String content = String.format("Hồ sơ tiêm chủng của học sinh '%s' vừa được cập nhật bởi nhân viên y tế.", studentOfRecord.getFullName());
            String link = "/vaccinations/" + existingVaccination.getStudentVaccinationId();
            if (studentOfRecord.getParentLinks() != null) {
                studentOfRecord.getParentLinks().forEach(linkObj -> {
                    User parent = linkObj.getParent();
                    if (parent != null && parent.getEmail() != null) {
                        ...
                        log.info("[TIÊM CHỦNG] Đã gửi thông báo cho phụ huynh {} về việc cập nhật hồ sơ tiêm chủng của học sinh ID: {}", parent.getEmail(), studentId);
                    }
                });
            }
        }
    ...
    public StudentVaccinationResponseDto mediateVaccinationStatusForCurrentUser(Long vaccinationId, VaccinationStatusUpdateRequestDto statusUpdateRequestDto) {
        log.info("[TIÊM CHỦNG] Bắt đầu duyệt bản ghi tiêm chủng ID: {} bởi người dùng hiện tại.", vaccinationId);
        User currentUser = authorizationService.getCurrentUserAndValidate();
        ...
        log.info("[TIÊM CHỦNG] Bản ghi ID: {}. Trạng thái hiện tại: {}. Trạng thái mới yêu cầu: {}", vaccinationId, currentStatus, requestedNewStatus);
        ...
        log.info("[TIÊM CHỦNG] Đã cập nhật trạng thái cho bản ghi tiêm chủng ID {} thành {}. Duyệt bởi: {}. Ghi chú: {}",
        ...
        sendNotificationToParent(updatedVaccination, statusUpdateRequestDto.approverNotes());

        return vaccinationMapper.toDto(updatedVaccination);
    }

    private void sendNotificationToParent(StudentVaccination vaccination, String reason) {
        try {
            Student student = vaccination.getStudent();
            if (student == null || student.getParentLinks() == null || student.getParentLinks().isEmpty()) {
                log.warn("[TIÊM CHỦNG] Không tìm thấy thông tin phụ huynh cho học sinh ID: {} để gửi thông báo.", student != null ? student.getId() : "null");
                return;
            }
            ...
            if (statusMessage != null) {
                ...
                student.getParentLinks().forEach(link -> {
                    ...
                    if (parent != null && parent.getEmail() != null) {
                        ...
                        log.info("[TIÊM CHỦNG] Đã gửi thông báo về hồ sơ tiêm chủng ID {} tới phụ huynh: {}", vaccination.getStudentVaccinationId(), parentUsername);
                    }
                });
            }
        } catch (Exception e) {
            log.error("[TIÊM CHỦNG] Lỗi khi gửi thông báo WebSocket đến phụ huynh: {}", e.getMessage(), e);
        }
    }

    @Transactional
    public void deleteVaccinationForCurrentUser(Long vaccinationId) {
        log.info("[TIÊM CHỦNG] Bắt đầu yêu cầu xóa bản ghi tiêm chủng ID: {} bởi người dùng hiện tại.", vaccinationId);
        User currentUser = authorizationService.getCurrentUserAndValidate();
        StudentVaccination vaccination = vaccinationRepository.findById(vaccinationId)
                .orElseThrow(() -> new ResourceNotFoundException("Bản ghi tiêm chủng không tồn tại với ID: " + vaccinationId));
        ...
        // Xử lý quyền xóa cho phụ huynh: chỉ được xóa khi trạng thái là PENDING
        if (currentUserRole == UserRole.Parent) {
            ...
            if (vaccination.getStatus() != StudentVaccinationStatus.PENDING) {
                throw new AccessDeniedException("Phụ huynh chỉ có thể xóa thông tin tiêm chủng đang ở trạng thái 'Chờ xử lý'.");
            }
        }
        ...
        log.info("[TIÊM CHỦNG] Đã xóa thành công bản ghi tiêm chủng ID: {} bởi người dùng {}", vaccinationId, currentUser.getEmail());

        if (studentOfRecord.getParentLinks() != null) {
            String vaccineName = vaccination.getVaccineName();
            String studentName = studentOfRecord.getFullName();
            String deletedBy = currentUser.getEmail();
            String notificationContent = String.format("Vắc xin '%s' của học sinh '%s' đã bị xóa khỏi hệ thống bởi %s.", vaccineName, studentName, deletedBy);
            String linkToResource = "/vaccinations";
            studentOfRecord.getParentLinks().forEach(link -> {
                User parent = link.getParent();
                if (parent != null && parent.getEmail() != null) {
                    notificationService.createAndSendNotification(parent.getEmail(), notificationContent, linkToResource, deletedBy);
                    log.info("[TIÊM CHỦNG] Đã gửi thông báo xóa tới phụ huynh: {}", parent.getEmail());
                }
            });
        }
    }
    ...
    public String getSignedUrlForProofFile(Long vaccinationId) {
        log.info("[TIÊM CHỦNG] Yêu cầu tạo signed URL cho file bằng chứng của bản ghi tiêm chủng ID: {}", vaccinationId);
        User currentUser = authorizationService.getCurrentUserAndValidate();
        StudentVaccination entity = vaccinationRepository.findById(vaccinationId)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy bản ghi tiêm chủng với ID: " + vaccinationId));
        ...
        if (entity.getProofPublicId() == null || entity.getProofPublicId().isEmpty()) {
            log.warn("[TIÊM CHỦNG] Bản ghi tiêm chủng ID: {} không có thông tin file bằng chứng (publicId rỗng).", vaccinationId);
            return null;
        }
        ...
        if (signedUrl == null) {
            log.error("[TIÊM CHỦNG] Không thể tạo signed URL cho public_id: {} của bản ghi ID: {}", entity.getProofPublicId(), vaccinationId);
            throw new FileStorageException("Lỗi khi tạo URL truy cập file. Vui lòng thử lại sau.");
        }
        log.info("[TIÊM CHỦNG] Đã tạo signed URL thành công cho public_id: {} của bản ghi ID: {}", entity.getProofPublicId(), vaccinationId);
        return signedUrl;
    }
}
