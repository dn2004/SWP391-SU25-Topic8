...
public class AuthService {
    ...
    public UserDto registerParent(RegisterParentRequestDto dto) {
        log.info("[AUTH] Đăng ký tài khoản phụ huynh mới cho email: {}", dto.email());
        if (userRepository.existsByEmail(dto.email())) {
            log.warn("[AUTH] Email {} đã tồn tại.", dto.email());
        ...
        log.info("[AUTH] Đã tạo tài khoản phụ huynh thành công: {}", savedParent.getEmail());
    ...
    private void sendNewParentRegistrationNotification(User parent) {
        try {
            ...
            log.info("[AUTH] Đã gửi thông báo đăng ký phụ huynh mới tới SchoolAdmin cho tài khoản: {}", parent.getEmail());
        } catch (Exception e) {
            log.error("[AUTH] Lỗi khi gửi thông báo đăng ký phụ huynh mới cho tài khoản {}: {}", parent.getEmail(), e.getMessage(), e);
        }
    }

    public LoginResponseDto login(LoginRequestDto dto) {
        log.info("[AUTH] Xác thực người dùng: {}", dto.email());
        try {
        ...
        User user = (User) authentication.getPrincipal();
            ...
            if (!user.isActive()) {
                log.warn("[AUTH] Tài khoản {} chưa được kích hoạt hoặc đã bị vô hiệu hóa.", dto.email());
        ...
        log.info("[AUTH] Đăng nhập thành công cho: {}", user.getEmail());
        UserDto userDto = userMapper.userToUserDto(user);

        return new LoginResponseDto(jwt, userDto);
        } catch (AuthenticationException e) {
            log.warn("[AUTH] Lỗi xác thực cho người dùng {}: {}", dto.email(), e.getMessage());
    ...
    public void logout(HttpServletRequest request) {
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            log.info("[AUTH] Không có token để logout.");
            return;
        }
        final String jwt = authHeader.substring(7);
        if (!StringUtils.hasText(jwt) || !jwtService.validateToken(jwt)) {
            log.warn("[AUTH] Token không hợp lệ hoặc đã hết hạn, không cần blacklist.");
            return;
        }
        ...
        try {
            ...
            if (claims == null) {
                log.warn("[AUTH] Không thể lấy claims từ token hợp lệ, không thể blacklist JTI.");
                ...
                if (expiresIn > 0) {
                    ...
                    log.info("[AUTH] Token (string) đã được blacklist. Key: {}{}", blacklistPrefix, jwt.substring(0,10)+"...");
                }
                return;
            }
            ...
            if (jti != null && expirationTime != null) {
                ...
                if (expiresIn > 0) {
                    ...
                    log.info("[AUTH] Token với JTI {} đã được blacklist.", jti);
                } else {
                    log.info("[AUTH] Token với JTI {} đã hết hạn, không cần blacklist.", jti);
                }
            } else {
                log.warn("[AUTH] Không tìm thấy JTI hoặc thời gian hết hạn trong token, không thể blacklist bằng JTI.");
            ...
            log.error("[AUTH] Lỗi khi blacklist token: {}", e.getMessage(), e);
            ...
            try {
                ...
                if (claimsForBlacklist != null && claimsForBlacklist.getExpirationTime() != null) {
                    ...
                    if (expiresIn > 0) {
                        ...
                        log.info("[AUTH] Token (string fallback) đã được blacklist. Key: {}{}", blacklistPrefix, jwt.substring(0,10)+"...");
                    }
                }
            } catch (Exception ex) {
                log.error("[AUTH] Lỗi nghiêm trọng khi cố gắng blacklist token (fallback): {}", ex.getMessage(), ex);
            }
        }
    }

    public void forgotPassword(ForgotPasswordRequestDto dto) {
        log.info("[AUTH] Yêu cầu quên mật khẩu cho email: {}", dto.email());
        otpService.generateAndSendOtp(dto.email());
        log.info("[AUTH] Đã gửi yêu cầu OTP cho {}", dto.email());
    }

    @Transactional
    public void resetPassword(ResetPasswordRequestDto dto) {
        log.info("[AUTH] Yêu cầu đặt lại mật khẩu cho email: {}", dto.email());
        otpService.resetPasswordWithOtp(dto.email(), dto.otp(), dto.newPassword());
        log.info("[AUTH] Mật khẩu đã được đặt lại thành công cho {}", dto.email());
    }
}