...
public class StringToUserRoleConverter implements Converter<String, UserRole> {
    ...
    public UserRole convert(@NonNull String source) {
        log.debug("Đang chuyển đổi chuỗi '{}' thành UserRole", source);

        if (source.isEmpty()) {
            log.warn("Chuỗi nguồn rỗng. Trả về null cho UserRole.");
            return null;
        }

        try {
            UserRole result = UserRole.fromDisplayName(source);
            log.debug("Đã chuyển đổi thành công '{}' thành enum '{}'", source, result);
            return result;
        } catch (IllegalArgumentException e) {
            log.error("Chuyển đổi không thành công cho chuỗi '{}' thành UserRole. Lỗi: {}",
                    source, e.getMessage());
            throw e;
        }
    }
