...
public record StudentVaccinationRequestDto(
        ...
        @Size(max = 100, message = "Tên vắc-xin tối đa 100 ký tự")
        String vaccineName,

        @Schema(description = "Ngày tiêm chủng", example = "2022-08-15", requiredMode = Schema.RequiredMode.REQUIRED)
        ...
        @Schema(description = "Nơi tiêm chủng", example = "Trung tâm Y tế Dự phòng Quận 1", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank(message = "Nơi tiêm chủng không được để trống")
        @Size(max = 200, message = "Nơi tiêm chủng tối đa 200 ký tự")
        String provider,

        @Schema(description = "Ghi chú thêm", example = "Phản ứng nhẹ sau tiêm")
